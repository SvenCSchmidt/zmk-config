 ...
 * pure 3x5 + 2-thumbs/side core numbered CONTIGUOUSLY 0..33 (row-major, both hands
 * per row; thumbs 30/31 left, 32/33 right). This is by far the most common split/
 * unibody layout. A board with this exact matrix order includes this header
 * directly instead of carrying its own copy; a board that differs even slightly
 * gets its own geom_<board>.h.
 *
 ...
 *           30 31    |  32 33                 thumbs
 *
 * Boards currently using it: re-gret. (cb34s and Urchin have identical matrices and
 * could be migrated to this header too.)
 */

#pragma once

/* --- Symbolic key positions (logical core -> contiguous numbers) --- */
/* row 0 */
#define POS_LT0 0
#define POS_LT1 1
#define POS_LT2 2
 ...
 * Emits only the master core slots (identity), ignoring every optional master slot
 * (OUT/INX columns, FN/NUM/EXT rows, TH_O thumbs). */
#define LAYOUT( \
